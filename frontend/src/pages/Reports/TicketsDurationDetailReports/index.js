...
const TicketsDurationDetailReports = () => {
  ...
  const handleSearch = (event, type) => {
    const validatedDate = validateAndFormatDate(event.target.value);
    if (type === 'start') setDateStartParam(validatedDate);
    else setDateEndParam(validatedDate);
  };
  ...
  useEffect(() => {
    ...
    if (location.state.state.dateStartParam) {
      setDateStartParam(validateAndFormatDate(location.state.state.dateStartParam));
    }

    if (location.state.state.dateEndParam) {
      setDateEndParam(validateAndFormatDate(location.state.state.dateEndParam));
    }
    }
  }, [location]);

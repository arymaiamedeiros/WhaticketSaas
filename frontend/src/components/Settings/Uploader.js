...
const Uploader = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      if (!user.super) {
        showToast.error("Sem permissão para acessar!");
        setTimeout(() => {
          history.push(`/`)
        }, 500);
      }
...
const handleFileChange = (event) => {
  ...
  if (selectedFile && allowedTypes.includes(selectedFile.type)) {
    ...
    showToast.error("Use somente arquivos em formato PNG, ICO ou SVG!");
  }
};
  ...
  const handleSubmit = async (event) => {
    ...
    if (!file) {
      showToast.warn("Escolha um arquivo!");
      return;
    }

    if (!selectedOption) {
      showToast.warn("Escolha um destino!");
      return;
    }
...
return (
  <>
    <Grid spacing={3} container>
      ...
      >
        <Tab value={0} label="Logotipos / Ícones" />
      </Tabs>
